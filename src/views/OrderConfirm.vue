            ...
            <div class="checkout-order">
                            ...
                            <li v-for="(item,index) in cartList" :key="index" v-if="item.checked === '1'">
                                <div class="cart-tab-1">
                                    <div class="cart-item-pic">
                                        <img v-lazy='$http.baseImgUrl+item.productImage' alt="">
                                    </div>
                ...
                <div class="order-foot-wrap">
                    <div class="prev-btn-wrap">
                        <button class="btn btn--m" @click="$router.replace({path:'/cart'})">修改订单</button>
                    </div>
                ...
                this.$http.GET('/api/users/cartList', {}, (respData) => {
                    if (respData.status === '0') {
                        ...
                        this.cartList.forEach((item)=>{
                            if(item.checked === '1'){
                                //计算总价
                                this.subTotal += parseFloat(item.productNum,10)*parseFloat(item.salePrice,10)
                            }
                            this.orderTotal = this.shipping - this.discount + this.tax + this.subTotal;
                        })
