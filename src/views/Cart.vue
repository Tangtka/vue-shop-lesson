                            ...
                            <li v-for="(item,index) in cartList" :key="index">
                                <div class="cart-tab-1">
                                    ...
                                    <div class="cart-item-pic">
                                        <img v-lazy='$http.baseImgUrl+item.productImage'>
                                    </div>
        ...
        computed: {
            ...
            totalPrice() {
                ...
                this.cartList.forEach((item) => {
                    if (item.checked === '1') {
                        money += parseFloat(item.salePrice, 10) * parseFloat(item.productNum, 10);
                    }
                });
                return money;
            },
        ...
        methods: {
            ...
            editCart(flag,item){
                ...
                this.$http.POST('/users/cartEdit', {
                    ...
                    if (respData.status === '0') {
                        this.$store.commit('updateCartCount',flag === 'add'? 1:flag === 'minu'? -1:0);
                        this.getCartList();
                    } else {
                        console.log(respData.msg)
                    }
